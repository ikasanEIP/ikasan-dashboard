 ...
 */
public class SolrScheduledProcessServiceImpl extends SolrServiceBase implements ScheduledProcessManagementService, ScheduledProcessEventService, SolrService<ScheduledProcessEvent>, BatchInsert<ScheduledProcessEvent>
{
    ...
    public List<FlowMetaData> getFlowsForAgent(String agent, String filter) {
        ModuleMetaData moduleMetaData = this.solrModuleMetadataDao.findById(agent);

        return moduleMetaData.getFlows().stream()
            .filter(flowMetaData -> (filter == null || flowMetaData.getName().toLowerCase().contains(filter.toLowerCase())) && !flowMetaData.getName().equals("Scheduled Process Event Outbound Flow"))
            .collect(Collectors.toList());
    }
            ...
            if(accessibleModules == null || accessibleModules.contains(agentName)) {
                ...
                moduleMetaData.getFlows().forEach(flowMetaData -> {
                    try {
                        ...
                        // todo ignore for now until model is up to dates
                    }
                });
            }
        });
    ...
    public ScheduledProcessEventSearchResults<ScheduledProcessAggregateConfiguration> getScheduleProcessAggregateConfigurations(String agent, String filter) {
        ...
        flows.forEach(flowMetaData -> {
            ScheduledProcessAggregateConfiguration configuration = this.getScheduleProcessAggregateConfiguration(agent, flowMetaData);
            // todo sort out null configuration after model defined
    ...
    public ScheduledProcessEventSearchResults<ScheduledProcessAggregateConfiguration> getScheduleProcessAggregateConfigurations(String agent, String filter, int offset, int limit) {
        ...
        if(flows.size() > offset + limit) {
            flows = flows.subList(offset, offset+limit);
        }
        else {
            flows = flows.subList(offset, flows.size());
        }
    ...
    @Override
    public ScheduledProcessAggregateConfiguration getScheduleProcessAggregateConfiguration(String agentName, FlowMetaData flowMetaData) {
        if(flowMetaData.getName().equals("Scheduled Process Event Outbound Flow")){
            // todo sort out when model complete
            return null;
        }
        AtomicReference<ScheduledProcessAggregateConfiguration> scheduledProcessAggregateConfiguration = new AtomicReference<>(new ScheduledProcessAggregateConfiguration());
        ...
        flowMetaData.getFlowElements().stream()
            .filter(flowElementMetaData -> (flowElementMetaData.getComponentName().equals("Scheduled Consumer")
                || flowElementMetaData.getComponentName().equals("File Consumer")))
            .findFirst()
            .map(flowElementMetaData -> flowElementMetaData.getConfigurationId())
            .ifPresent(id -> scheduledConsumerConfigurationId.set(id));
        ...
        scheduledProcessAggregateConfiguration.set(this.scheduledProcessAggregateConfigurationConverter.convert(bucket));
        scheduledProcessAggregateConfiguration.get().setAgentName(agentName);
        scheduledProcessAggregateConfiguration.get().setJobName(flowMetaData.getName());
        scheduledProcessAggregateConfiguration.get().setStartAutomatically(flowMetaData.getFlowStartupType().equals("AUTOMATIC"));
//        scheduledProcessAggregateConfiguration.get().setBusinessStreamMetaData(this.solrBusinessStreamMetadataDao
//            .findBusinessStreamsContainingFlow(agentName, flowMetaData.getName(), 0, 1000));
