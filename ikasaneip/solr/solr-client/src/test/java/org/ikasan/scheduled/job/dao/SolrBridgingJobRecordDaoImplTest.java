...
import org.ikasan.scheduled.job.model.SolrBridgingJobImpl;
...
public class SolrBridgingJobRecordDaoImplTest extends SolrTestCaseJ4 {

    SolrBridgingJobDaoImpl dao;
    ...
    private void init(EmbeddedSolrServer server) throws IOException, SolrServerException
    {
        ...
        dao = new SolrBridgingJobDaoImpl();
    ...
    public void test_save_and_find_success() throws Exception {
        ...
        try (EmbeddedSolrServer server = new EmbeddedSolrServer(config, "ikasan"))
        {
            ...
            BridgingJobRecord found = this.dao.findById("BRIDGING_JOB_jobName0_contextId");

            Assert.assertEquals("BRIDGING_JOB_jobName0_contextId", found.getId());
    ...
    private void insertRecords(String idPrefix, int num, String contextId) {
        IntStream.range(0, num).forEach(i -> {
            BridgingJob solrBridgingJob = new SolrBridgingJobImpl();
            solrBridgingJob.setAgentName(idPrefix+"agentName"+i);
            solrBridgingJob.setJobName("jobName"+i);
            solrBridgingJob.setContextName(contextId);

            BridgingJobRecord solrBridgingJobRecord = new SolrBridgingJobRecordImpl();
            solrBridgingJobRecord.setAgentName(idPrefix+"agentName"+i);
            solrBridgingJobRecord.setJobName("jobName"+i);
            solrBridgingJobRecord.setContextName(contextId);
            solrBridgingJobRecord.setTimestamp(1000000L);
            solrBridgingJobRecord.setBridgingJob(solrBridgingJob);


            this.dao.save(solrBridgingJobRecord);
        });
    }
    ...
    private void insertRecordsAsList(String idPrefix, int num, String contextId) {
        ...
        IntStream.range(0, num).forEach(i -> {
            ...
            records.add(solrBridgingJobRecord);
        });
