...
public class SolrEmailNotificationContextServiceImplTest extends SolrTestCaseJ4 {

    private SolrEmailNotificationContextDaoImpl dao;
    private SolrEmailNotificationContextServiceImpl service;
    ...
    private void init(EmbeddedSolrServer server) throws IOException, SolrServerException {
        ...
        service = new SolrEmailNotificationContextServiceImpl(dao);
    }
    ...
    public void test_save_and_find_success() throws Exception {
        ...
        try (EmbeddedSolrServer server = new EmbeddedSolrServer(config, "ikasan"))
        {
            ...
            this.service.save(solrEmailNotificationContextRecord);

            SearchResults<EmailNotificationContextRecord> found = this.service.findByContextName("parent-context-1", 100, 0);
    ...
    public void test_save_and_find_all() throws Exception {
        ...
        try (EmbeddedSolrServer server = new EmbeddedSolrServer(config, "ikasan")) {
            ...
            // One using saveEmailNotificationContext
            this.service.saveEmailNotificationContext(emailNotificationContext);
            ...
            // One using save
            this.service.save(solrEmailNotificationContextRecord2);

            SearchResults<EmailNotificationContextRecord> found = this.service.findAll(100, 0);
            ...
            for (EmailNotificationContextRecord foundResults : found.getResultList()) {
                if (foundResults.getId().equals("parent-context-1")) {
                    Assert.assertEquals("parent-context-1", foundResults.getId());
                    Assert.assertEquals("parent-context-1", foundResults.getContextName());
                    Assert.assertNull(foundResults.getModifiedBy());
                    EmailNotificationContext foundEmailContext = foundResults.getEmailNotificationContext();
                    Assert.assertEquals("parent-context-1", foundEmailContext.getContextName());
                    Assert.assertTrue(foundEmailContext.getMonitorTypes().contains("ERROR"));
                    Assert.assertEquals("someBodyLocation.txt", foundEmailContext.getEmailBodyNotificationTemplate().get("ERROR"));
    ...
    public void test_delete_context_name() throws Exception {
        ...
        try (EmbeddedSolrServer server = new EmbeddedSolrServer(config, "ikasan"))
        {
            ...
            this.service.saveEmailNotificationContext(emailNotificationContext2);
            ...
            this.service.deleteByContextName("parent-context-2");
            ...
            this.service.deleteByContextName("parent-context-1");
            found = this.service.findAll( 100, 0);
