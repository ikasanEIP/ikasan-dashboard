...
public class SolrContextTerminalJobDaoImplTest extends SolrTestCaseJ4 {
    ...
    private void init(EmbeddedSolrServer server) throws IOException, SolrServerException
    {
        ...
        dao = new SolrContextTerminalJobDaoImpl();
    ...
    public void test_save_and_find_success() throws Exception {
        ...
        try (EmbeddedSolrServer server = new EmbeddedSolrServer(config, "ikasan"))
        {
            ...
            ContextTerminalJobRecord found = this.dao.findById("CONTEXT_TERMINAL_JOB_jobName0_contextId");

            Assert.assertEquals("CONTEXT_TERMINAL_JOB_jobName0_contextId", found.getId());
            Assert.assertEquals("CONTEXT_TERMINAL_JOB", found.getAgentName());
    ...
    private void insertRecords(String idPrefix, int num, String contextId) {
        IntStream.range(0, num).forEach(i -> {
            ContextTerminalJob solrContextTerminalJob = new SolrContextTerminalJobImpl();
            solrContextTerminalJob.setAgentName(idPrefix+"agentName"+i);
            solrContextTerminalJob.setJobName("jobName"+i);
            solrContextTerminalJob.setContextName(contextId);

            ContextTerminalJobRecord solrContextTerminalJobRecord = new SolrContextTerminalJobRecordImpl();
    ...
    private void insertRecordsAsList(String idPrefix, int num, String contextId) {
        ...
        IntStream.range(0, num).forEach(i -> {
            ...
            records.add(solrContextTerminalJobRecord);
        });
