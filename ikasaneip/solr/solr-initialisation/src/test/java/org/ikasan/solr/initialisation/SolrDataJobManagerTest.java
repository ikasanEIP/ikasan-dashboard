...
import org.ikasan.solr.initialisation.core.SolrInitialDataJobStatusConstants;
...
/**
 * Unit test for SolrDataJobManager
 */
public class SolrDataJobManagerTest {
    ...
    private SolrDataJob mockJob1;
    private SolrDataJob mockJob2;
    private SolrDataJob mockJob3;

    @Before
    public void setup() {
        mockSetupService = mockery.mock(SetupService.class);
        mockJob1 = mockery.mock(SolrDataJob.class, "job1");
        mockJob2 = mockery.mock(SolrDataJob.class, "job2");
        mockJob3 = mockery.mock(SolrDataJob.class, "job3");
    }

    @Test(expected = IllegalArgumentException.class)
    public void test_constructor_throws_exception_when_setupService_is_null() {
        new SolrDataJobManager(null, new ArrayList<>());
    }

    @Test(expected = IllegalArgumentException.class)
    public void test_constructor_throws_exception_when_solrDataJobs_is_null() {
        new SolrDataJobManager(mockSetupService, null);
    }

    @Test
    public void test_constructor_accepts_valid_parameters() {
        List<SolrDataJob> jobs = Arrays.asList(mockJob1, mockJob2);
        SolrDataJobManager manager = new SolrDataJobManager(mockSetupService, jobs);
    ...
    public void test_execute_skips_jobs_already_completed() throws Exception {
        ...
        SolrDashboardPlatformSetupImpl existingSetup = new SolrDashboardPlatformSetupImpl();
        List<DashboardSetupItem> items = new ArrayList<>();
        items.add(new SolrDashboardSetupItemImpl("JOB_1",
    ...
    public void test_execute_runs_job_with_error_status() throws Exception {
        ...
        List<DashboardSetupItem> items = new ArrayList<>();
        items.add(new SolrDashboardSetupItemImpl("JOB_1",
            SolrInitialDataJobStatusConstants.ERROR, System.currentTimeMillis()));
        existingSetup.setPlatformSetupItems(items);

        List<SolrDataJob> jobs = Arrays.asList(mockJob1);
        SolrDataJobManager manager = new SolrDataJobManager(mockSetupService, jobs);
    ...
    public void test_execute_with_empty_job_list() throws Exception {
        List<SolrDataJob> jobs = new ArrayList<>();
        SolrDataJobManager manager = new SolrDataJobManager(mockSetupService, jobs);
    ...
    public void test_execute_saves_setup_with_correct_status() throws Exception {
        ...
        Assert.assertEquals(SolrInitialDataJobStatusConstants.COMPLETE_SUCCESS,
    ...
    @Test(expected = SolrDataJobException.class)
    ...
    public void test_execute_processes_multiple_jobs_in_order() throws Exception {
        ...
        List<SolrDataJob> jobs = Arrays.asList(mockJob1, mockJob2, mockJob3);
        SolrDataJobManager manager = new SolrDataJobManager(mockSetupService, jobs);
