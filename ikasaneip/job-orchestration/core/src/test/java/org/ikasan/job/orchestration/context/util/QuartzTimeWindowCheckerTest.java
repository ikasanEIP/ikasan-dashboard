...
public class QuartzTimeWindowCheckerTest {
    ...
    public void is_within_an_operating_window_of_5_minutes() {
        ...
        assertFalse(QuartzTimeWindowChecker.withinOperatingWindow("0 35 14 ? * * *", "0 30 14 ? * * *", new Date(now.getTime())));
        assertTrue(QuartzTimeWindowChecker.outsideOfOperatingWindow("0 35 14 ? * * *", "0 30 14 ? * * *", new Date(now.getTime())));

        now = Timestamp.valueOf(LocalDateTime.of(2022, 5, 18, 14, 35, 0, 0));
        assertTrue(QuartzTimeWindowChecker.withinOperatingWindow("0 35 14 ? * * *", "0 30 14 ? * * *", new Date(now.getTime())));
        assertFalse(QuartzTimeWindowChecker.outsideOfOperatingWindow("0 35 14 ? * * *", "0 30 14 ? * * *", new Date(now.getTime())));
