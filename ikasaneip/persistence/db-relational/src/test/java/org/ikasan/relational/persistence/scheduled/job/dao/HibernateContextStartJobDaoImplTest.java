...
/**
 * Integration test for HibernateContextStartJobDaoImpl using Testcontainers with PostgreSQL.
 */
...
public class HibernateContextStartJobDaoImplTest {
    ...
    @Autowired
    private ContextStartJobDao<HibernateContextStartJobRecord> dao;
    ...
    public void tearDown() {
        ...
        SearchResults<HibernateContextStartJobRecord> all = dao.findAll(-1, -1);
    ...
    private ContextStartJob createContextStartJob(String jobName, String contextName) {
        return new TestContextStartJob(jobName, contextName);
    }
    ...
    private HibernateContextStartJobRecord createContextStartJobRecord(String jobName, String contextName, String agentName) {
        ...
        record.setContextStartJob(createContextStartJob(jobName, contextName));
    ...
    public void testSaveAndFindById() {
        // Given
        HibernateContextStartJobRecord record = createContextStartJobRecord("test-job-1", "test-context-1", "agent-1");

        // When
        dao.save(record);

        // Then
        HibernateContextStartJobRecord found = dao.findById(record.getId());
        assertNotNull(found);
        ...
        assertNotNull(found.getContextStartJob());
        assertEquals("test-job-1", found.getContextStartJob().getJobName());
    }

    @Test
    public void testFindByIdNotFound() {
        // When
        HibernateContextStartJobRecord result = dao.findById("non-existent-id");
    ...
    public void testSaveUpdatesTimestamps() {
        // Given
        HibernateContextStartJobRecord record = createContextStartJobRecord("test-job-2", "test-context-2", "agent-2");
    ...
    public void testSaveAndUpdate() throws InterruptedException {
        // Given
        HibernateContextStartJobRecord record = createContextStartJobRecord("test-job-3", "test-context-3", "agent-3");
    ...
    public void testFindAll() {
        // Given
        dao.save(createContextStartJobRecord("job-1", "context-1", "agent-1"));
        dao.save(createContextStartJobRecord("job-2", "context-2", "agent-2"));
        dao.save(createContextStartJobRecord("job-3", "context-3", "agent-3"));

        // When
        SearchResults<HibernateContextStartJobRecord> results = dao.findAll(-1, -1);
    ...
    public void testFindAllWithLimitAndOffset() {
        // Given
        for (int i = 1; i <= 5; i++) {
            dao.save(createContextStartJobRecord("find-job-" + i, "context-" + i, "agent-" + i));
        }

        // When
        SearchResults<HibernateContextStartJobRecord> results = dao.findAll(2, 1);
    ...
    public void testFindByContext() {
        ...
        dao.save(createContextStartJobRecord("job-1", contextName, "agent-1"));
        dao.save(createContextStartJobRecord("job-2", contextName, "agent-2"));
        dao.save(createContextStartJobRecord("job-3", "other-context", "agent-3"));

        // When
        SearchResults<HibernateContextStartJobRecord> results = dao.findByContext(contextName, -1, -1);
    ...
    public void testFindByContextWithLimitAndOffset() {
        ...
        for (int i = 1; i <= 5; i++) {
            dao.save(createContextStartJobRecord("job-" + i, contextName, "agent-" + i));
        }

        // When
        SearchResults<HibernateContextStartJobRecord> results = dao.findByContext(contextName, 2, 1);
    ...
    public void testFindByContextNotFound() {
        // When
        SearchResults<HibernateContextStartJobRecord> results = dao.findByContext("non-existent-context", -1, -1);
    ...
    public void testJsonSerializationDeserialization() {
        // Given
        HibernateContextStartJobRecord record = createContextStartJobRecord("json-job", "json-context", "json-agent");
        ContextStartJob contextStartJob = record.getContextStartJob();
        contextStartJob.setJobDescription("Test description");
        contextStartJob.setDisplayName("Test Display");
        contextStartJob.setStartupControlType("MANUAL");
        ...
        assertNotNull(found);
        assertNotNull(found.getContextStartJob());
        assertEquals("Test description", found.getContextStartJob().getJobDescription());
        assertEquals("Test Display", found.getContextStartJob().getDisplayName());
        assertEquals("MANUAL", found.getContextStartJob().getStartupControlType());
    }

    @Test(expected = IllegalArgumentException.class)
    public void testSaveInvalidRecordType() {
        // This test validates that the DAO only accepts HibernateContextStartJobRecord
    ...
    private static class TestContextStartJob implements ContextStartJob {
        ...
        public TestContextStartJob(String jobName, String contextName) {
